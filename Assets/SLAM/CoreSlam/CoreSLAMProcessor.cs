...
namespace CoreSLAM
{
    ...
    public class CoreSLAMProcessor : IDisposable
    {
        ...
        //    v128 sincos = new v128(c, -s, s, c);
        //    v128 posxy = new v128(px, py, px, py);

        //    // Translate and rotate scan to robot position and compute the "distance"
        //    for (int i = 0; i < cloud.Points.Count; i++)
        //    {
        //        v128 xy = new v128(cloud.Points[i].X, cloud.Points[i].Y, cloud.Points[i].X, cloud.Points[i].Y);
        //        xy = Sse4_1.mul_epi32(sincos, xy);
